  ...
  return (
    <div className="space-y-8">
      <section className="rounded-2xl border border-[#C9A227]/40 bg-[#0a0a0a] px-8 py-12 text-[#F5F0E6] shadow-lg">
        <p className="font-util text-sm font-semibold uppercase tracking-[0.2em] text-[#C9A227]">
          Krewe &amp; Kin studio
        </p>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link
            href="/clients/login"
            className="rounded-md bg-[#C9A227] px-4 py-2.5 font-util text-sm font-semibold uppercase tracking-[0.12em] text-[#0a0a0a] hover:bg-[#e0b93a]"
          >
            Client sign in
          </Link>
          <a
            href="mailto:missy@kreweandkin.com"
            className="rounded-md border border-[#F5F0E6]/40 px-4 py-2.5 font-util text-sm font-semibold uppercase tracking-[0.12em] hover:border-[#C9A227]"
          >
            Contact Missy
          </a>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        {[
          ...
          ["PayPal checkout", "Pay deposits and balances securely online with PayPal."],
        ].map(([title, body]) => (
          <div
            key={title}
            className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm"
          >
            <h2 className="text-[#0a0a0a]">{title}</h2>
